...
const Stake = () => {
  ...
  const getWblurBalance = async () => {
    try {
      ...
      setUserWblurBalance(
        Number(BigInt(res._hex) / 10n ** BigInt(res2 - 2)) / 100
      );
    } catch (error) {
      console.log(error);
    }
  ...
  const checkApprove = async () => {
    try {
      ...
      setBlurAllowance(Number(BigInt(res._hex) / 10n ** 16n) / 100);
      console.log(
        "blur allowance",
        Number(BigInt(res._hex) / 10n ** 16n) / 100
      );
      if (res == 0) {
      }
    } catch (error) {
